...
public class Application1 {
    ...
    public static void main(String[] args) {
        ...
        *  2. Set 인터페이스
        *  - 순서가 없는 데이터 집합으로 데이터의 중복 저장을 허용하지 않는다.
        *  - HashSet, TreeSet 등이 있다.
        *  3. Map 인터페이스
        *  - 키와 값 한 쌍으로 이루어지는 데이터 집합이다.
        *  - key를 Set 방식으로 관리하기 때문에 데이터의 순서를 관리하지 않고 중복된 key를 혀용하지 않는다.
        ...
        *  JDK 1.2 부터 제공한다.
        *  내부적으로는 배열을 이용하여 요소를 관리하며, 인덱스를 이용해 배열 요소에 빠르게 접근할 수 있다.
        *
        *  ArrayList는 배열의 단점을 보완하기 위해 만들어졌다.
        *  배열은 크기를 변경할 수 없고, 요소의 추가, 삭제, 정력 등이 복잡하다는 단점을 가지고 있다.
        ...
        * */
        ...
        ArrayList alist = new ArrayList();      // 10칸짜리 배열이 생성됨
        ...
        for (int i = 0; i < alist.size(); i++) {
            System.out.println(i + " : " + alist.get(i));  // 인덱스에 해당하는 값만 가져올 때 get 이라는 메소드 사용
        }
...
//        stringList.add(123);
        ...
        Collections.sort(stringList);  // sort 메소드를 이용하여 오름차순 정렬
        System.out.println("stringList = " + stringList);

        stringList = new LinkedList<>(stringList);
//        System.out.println("stringList = " + stringList);     // 내림차순 정렬 하는건데 어째선지 안된다ㅠㅠ
